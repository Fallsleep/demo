...
(function ($) {
	$.jstree.plugin("aclcheckbox", {
		__init : function () {
			...
			this.get_container()
				...
				.bind("loaded.jstree", $.proxy(function (e) {
						...
						//插入样式
						var url = "js/acl/style/style.css";
						var css = false;
		...
		_fn : {
			_prepare_checkboxes : function (obj) {//重新定义checkbox状态
			...
			permit_node : function (obj) {//把节点与所有祖先节点设置为许可状态
				obj = this._get_node(obj);
				obj.removeClass("jstree-normal jstree-deny jstree-extends").addClass("jstree-permit");
				obj.parentsUntil(".jstree", "li").each(function(){
					var t = $(this);
					t.removeClass("jstree-normal jstree-deny jstree-extends").addClass("jstree-permit");
				});
			},
			deny_node : function (obj) {//把节点设置成拒绝状态
				obj = this._get_node(obj);
				obj.removeClass("jstree-normal jstree-permit jstree-extends").addClass("jstree-deny");
			},
			extends_node : function (obj) {//把节点设置成集成状态
				obj = this._get_node(obj);
				obj.addClass("jstree-extends");
			},
			cancel_node : function (obj) {//把节点设置成取消庒茹
				obj = this._get_node(obj);
				obj.removeClass("jstree-permit jstree-deny jstree-extends").addClass("jstree-normal");
			},
			permit_all : function(node)	{//指定节点或所有节点设置成许可状态
				var _this = this;
				if(node == null){
			...
			deny_all : function(node)	{//指定节点或所有节点设置拒绝可状态
				var _this = this;
				if(node == null){
					node = this.get_container();
					node.find("li").each(function(){
			...
			extends_all : function(node)	{//指定节点或所有节点设置成继承状态
				var _this = this;
				if(node == null){
					node = this.get_container();
					node.find("li").each(function(){
			...
			cancel_all : function(node)	{//指定节点或所有节点设置成取消状态
				var _this = this;
				if(node == null){
					node = this.get_container();
					node.find("li").each(function(){
			...
			get_all_auths_nodes : function(){//返回树本身被授权的节点(授权许可、拒绝、继承)
